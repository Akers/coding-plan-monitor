...
系统 SHALL 实现 MiniMax TokenPlan 适配器，通过 REST API 获取文本生成和图像生成的额度数据。适配器 SHALL 调用实测可用的 `https://www.minimaxi.com/v1/token_plan/remains` 端点并正确解析 `model_remains` 数组格式。
...
#### Scenario: MiniMax 周额度固定生成
- **WHEN** MiniMax API 返回文本生成或图像生成计划
- **THEN** 适配器 SHALL 为每个已识别计划生成 5 小时窗口额度 metric 和周额度 metric

#### Scenario: MiniMax 周额度为零
- **WHEN** `current_weekly_total_count` 为 0
- **THEN** 适配器 SHALL 生成对应周额度 metric，并将周额度总量、已用量和百分比按 0 处理

#### Scenario: MiniMax 周额度字段缺失
- **WHEN** 已识别的 MiniMax 计划不包含 `current_weekly_total_count` 或 `current_weekly_usage_count`
- **THEN** 适配器 SHALL 生成对应周额度 metric，并将缺失字段按 0 处理
...
- **THEN** 请求 URL SHALL 为 `https://www.minimaxi.com/v1/token_plan/remains`
