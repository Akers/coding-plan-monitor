...
供应商管理选项卡 SHALL 列出所有支持的供应商，允许启用/禁用、配置 API Key 或 OAuth 登录。OAuth 登录 SHALL 正确打开外部浏览器。OAuth 服务的停止操作 MUST 具有幂等性（无运行中服务器时 SHALL 静默返回成功而非报错）。
...
#### Scenario: OAuth 首次点击登录授权
- **WHEN** 用户在未进行过 OAuth 授权的会话中首次点击"登录授权"按钮
- **THEN** 系统 SHALL 成功启动 OAuth 服务器并在浏览器中打开授权页面，stopOAuth 操作在无运行中服务器时 SHALL 静默返回成功
